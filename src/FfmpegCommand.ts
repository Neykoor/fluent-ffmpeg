...
export class FfmpegCommand extends EventEmitter {
  ...
  watermark(overlayPath: string, options: WatermarkOptions = {}): this {
    ...
    this.currentOutput.overlay = { inputIndex, x, y };
    return this;
  }
  ...
  private buildArguments(): string[] {
    ...
    for (const output of this.outputs) {
      ...
      if (output.overlay) {
        const mainIndex = 0;
        const overlayIndex = output.overlay.inputIndex;
        const overlayStage = `overlay=${output.overlay.x}:${output.overlay.y}`;

        const chain = output.videoFilters.length
          ? `[${mainIndex}:v]${output.videoFilters.join(',')}[wmsrc];[wmsrc][${overlayIndex}:v]${overlayStage}`
          : `[${mainIndex}:v][${overlayIndex}:v]${overlayStage}`;

        args.push('-filter_complex', chain);
      } else if (output.videoFilters.length) {
